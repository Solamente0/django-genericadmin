  ...
  (function ($) {
    var GenericAdmin = {
      ...
      prepareSelect: function (select) {
        var that = this,
          ...
          no_value,
          opt_group_css =
            'style="font-style:normal; font-weight:bold; color:#999; padding-left: 2px;"';
      ...
      getLookupUrl: function (cID) {
        return (
          "../../../" +
          this.url_array[cID][0] +
          "/" +
          this.getLookupUrlParams(cID)
        );
      },
      ...
      showLookupLink: function () {
        ...
        link =
          link +
          '<strong id="lookup_text_' +
          this.getFkId() +
          '" margin-left: 5px"><a target="_new" href="#"></a><span></span></strong>';
              ...
              $("#lookup_text_" + that.getFkId() + " span")
                ...
                .html(
                  "Error: " +
                    xhr.status +
                    " &ndash; " +
                    that.capFirst(xhr.statusText.toLowerCase()),
                );
              if (xhr.status === 404) {
                that.object_input.val("");
              } else {
                $("#lookup_text_" + that.getFkId() + " span").css(
                  "color",
                  "#f00",
                );
              }
    ...
    var InlineAdmin = {
      ...
      install: function (fields, url_array, popup_var) {
        ...
        $("#" + this.fields.prefix + "-group .add-row a").click(
          this.addHandler(),
        );
      },
      addHandler: function () {
        ...
        return function (e) {
          ...
          var added_fields = $.extend({}, that.fields),
            admin = $.extend({}, GenericAdmin);
          added_fields.number =
            $("#id_" + that.fields.prefix + "-TOTAL_FORMS").val() - 1;
          ...
          $(
            "#" +
              ...
              added_fields.number +
              " .inline-deletelink",
          ).click(that.removeHandler(that));
        };
      },
      removeHandler: function (that) {
        return function (e) {
          var parent_id, deleted_num, sub_admin;

          e.preventDefault();
          parent_id = $(e.currentTarget)
            .parents(".dynamic-" + that.fields.prefix)
            .first()
            .attr("id");
